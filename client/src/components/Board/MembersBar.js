...
function MembersBar({ board, currentUser, onClose, onUpdate }) {
  ...
  const getRoleColor = (role) => ({ owner: '#fee75c', admin: '#5865f2', member: 'var(--text-muted)', viewer: 'var(--text-muted)' })[role];
  ...
  const renderMember = (member, isOnline) => {
    ...
    return (
      <div key={u?._id} className="member-item" style={{ opacity: isOnline ? 1 : 0.5 }}>
        {/* Кликабельный аватар */}
        <div className="member-avatar-clickable" onClick={() => setViewProfileId(u?._id)}>
          {renderAvatar(u, 32, 'member-avatar')}
          {isOnline && <div className="member-online-dot" />}
        </div>

        <div className="member-info">
          <div className="member-name member-name-clickable"
            onClick={() => setViewProfileId(u?._id)}>
          ...
          <div className="member-role"
            style={{
              ...
              cursor: canManageRoles() && u?._id !== currentUser._id && member.role !== 'owner' ? 'pointer' : 'default'
            }}
            onClick={() => {
            ...
            {canManageRoles() && u?._id !== currentUser._id && member.role !== 'owner' &&
              <FiChevronDown size={10} style={{ marginLeft: 4 }} />}
          </div>
          {roleMenuFor === u?._id && (
